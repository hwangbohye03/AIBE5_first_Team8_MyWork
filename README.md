...
| 만화책 반납 | 명령 인자로 넘어온 `rentalId` 존재 여부 확인 후 <br> 연결된 Comic 데이터 수량을 1 증가시키고 Rental 데이터를 반납 상태로 변경하고 DB 업데이트 |
...
| 기능 | 설명 |
|---|---|
| DB 연결 | `.env` 환경변수(`DB_URL`, `DB_USER`, `DB_PASSWORD`) 기반 DB 연결 생성 |
...
Rental 테이블은 대여 관계를 표현하기 위해 **외래키(Foreign Key)** 를 사용합니다.
